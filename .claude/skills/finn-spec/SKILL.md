---
name: finn-spec
description: Interview the user about a raw idea until confident, then file a build-ready GitHub issue. Use when asked to run Finn-loop's spec interview, draft a queue-ready issue, or plan a feature. Interactive — requires the user present; never run unattended.
---

# Spec interview (local setup — GitHub Issues)
...
issue on this repository using the GitHub MCP tools or `gh issue create`
with the draft as the body. Report the exact issue number and URL.

## Hard rule

Never apply the `agent-ready` label. The user applies it after a final
read — that label is the approval gate between "idea" and "an agent builds
it".
