---
name: finn-review
description: Review open PRs against their linked GitHub issues and required checks, then post a three-group verdict with Finn-loop labels. Use when asked to run Finn-loop's reviewer or review its PR queue. Designed for /loop; never merges or pushes code.
---

# Finn-loop reviewer (local setup — GitHub Issues)
...
- Parse the linked issue number from `Closes #NNN` in the PR body and
  fetch the full GitHub issue, including comments. No linked issue is a
  must-fix finding.
